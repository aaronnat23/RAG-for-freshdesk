...
class ImprovedAggregateAnalysis:
    ...
    def approach_1_semantic_search_analysis(self, query: str, max_tickets: int = 15) -> str:
        ...
        try:
            ...
            # Get relevant tickets data
            relevant_tickets = []
            for idx, similarity in zip(indices[0], similarities[0]):
                if similarity > 0.3:  # Only include reasonably similar tickets
            ...
            if not relevant_tickets:
                return "No tickets found relevant to your query."
            ...
            analysis_prompt = f"""
            You are analyzing {len(relevant_tickets)} tickets that are semantically similar to the query: "{query}"
            
            Find patterns, trends, and insights across these tickets:
            
            {combined_context}
            
            Provide analysis focusing on:
            1. Common patterns and themes
            2. Frequency of different issues
    ...
    def approach_2_clustering_analysis(self, selected_ticket_ids: list, num_clusters: int = 3) -> str:
        """
        Approach 2: Cluster tickets by similarity, analyze each cluster
        Groups similar tickets automatically
        """
        try:
            if len(selected_ticket_ids) < num_clusters:
                num_clusters = len(selected_ticket_ids)
            
            # Get embeddings for selected tickets
            embeddings = []
            ticket_data = []
            ...
            for ticket_id in selected_ticket_ids:
                try:
                    ...
                    embedding = self.vector_db.index.reconstruct(idx)
            ...
            if len(embeddings) < 2:
                return "Not enough tickets for clustering analysis."
            ...
            for i, label in enumerate(cluster_labels):
                if label not in clusters:
                    clusters[label] = []
                clusters[label].append(ticket_data[i])
            ...
            for cluster_id, tickets in clusters.items():
                ...
                cluster_prompt = f"""
                Analyze this cluster of {len(tickets)} similar tickets.
                Identify the main theme and common characteristics:
                
                {cluster_context}
                
                Focus on:
                1. What makes these tickets similar?
                2. Common issue types or patterns
                3. Typical resolution approaches
                """
                
                cluster_analysis = self.analyzer.analyze_with_llm(cluster_context, cluster_prompt)
                cluster_analyses.append(f"**Cluster {cluster_id + 1} ({len(tickets)} tickets):**\n{cluster_analysis}")
            
            # Combine cluster analyses
            final_analysis = "\n\n".join(cluster_analyses)
            
            metadata = f"\n\n📊 **Clustering Metadata:**\n"
            metadata += f"- Total tickets: {len(ticket_data)}\n"
            metadata += f"- Clusters formed: {len(clusters)}\n"
            for cluster_id, tickets in clusters.items():
                ticket_ids = [str(t['id']) for t in tickets]
                metadata += f"- Cluster {cluster_id + 1}: {len(tickets)} tickets ({', '.join(ticket_ids)})\n"
            
            return final_analysis + metadata
            
        except Exception as e:
            return f"Error in clustering analysis: {e}"
...
def main():
    ...
    try:
        ...
        st.warning(f"⚠️ sklearn not available for clustering analysis. Run 'pip install scikit-learn'. Error: {e}")
    ...
    with tab4:
        ...
        if not FAISS_AVAILABLE or not st.session_state.analyzer.vector_db or not st.session_state.analyzer.vector_db.ticket_ids:
            ...
            st.info("🚀 **Upgraded Analysis**: These methods leverage your vector database for smarter, faster analysis!")
            ...
            analysis_method = st.radio(
                ...
                help="""
                • **Semantic Search**: Finds tickets similar to your query, then analyzes them (1 LLM call)
                • **Clustering**: Groups similar tickets automatically, analyzes each group (3-5 LLM calls)  
                • **Pattern Mining**: Extracts patterns first, then AI interprets (1 LLM call)
            ...
            if analysis_method == "🔍 Semantic Search Analysis (Recommended)":
                ...
                if st.button("🚀 Run Semantic Analysis", type="primary", disabled=not agg_query):
                    with st.spinner("🔍 Finding relevant tickets and analyzing..."):
            ...
            elif analysis_method == "🎯 Clustering Analysis":
                if not SKLEARN_AVAILABLE:
                    st.error("❌ Clustering requires scikit-learn. Run 'pip install scikit-learn'")
                else:
                    all_ids = st.session_state.analyzer.vector_db.ticket_ids
                    selected_ids = st.multiselect(
                        "Select tickets to cluster and analyze:", 
                        all_ids, 
                        default=all_ids[:15] if all_ids else [],
                        help="Tickets will be automatically grouped by similarity"
                    )
                    
                    num_clusters = st.slider("Number of clusters", 2, 8, 3)
                    
                    if st.button("🎯 Run Clustering Analysis", type="primary", disabled=not selected_ids):
                        with st.spinner("🎯 Clustering tickets and analyzing each group..."):
                            start_time = time.time()
                            result = st.session_state.improved_analyzer.approach_2_clustering_analysis(
                                selected_ids, num_clusters
                            )
                            elapsed = time.time() - start_time
                            
                            st.success(f"✅ Clustering analysis completed in {elapsed:.1f} seconds!")
                            st.markdown(result)
                ...
                selected_ids = st.multiselect(
                    ...
                    help="Extracts patterns automatically, then uses AI for interpretation"
                )
